...
Each MUD file is accompanied by a signature file that verifies that the MUD file was indeed generated by the manufacturer. The MUD manager needs some basic information about each manufacturer.  This is used for both the "manufacturer" and "same-manufacturer" constructs.  In particular, any MUD file that references a "manufacturer" URL retrieves the associated IP address range from that entry to complete the rule.  If the entry is not available, the rule will be ignored.
